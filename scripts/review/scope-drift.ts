 ...
 */
export function analyzeDrift(plan: DriftPlan | null, diff: DriftDiff | null): DriftAnalysis {
  if (!plan || !diff) {
 ...
 */
export function formatDrift(analysis: DriftAnalysis): string {
 ...
 */
function extractIntent(todos?: string, prDesc?: string, commits?: string[]): string[] {
  const items: string[] = [];
 ...
 */
export function formatThreeWay(result: ThreeWayResult): string {
...
function matchesAnyWord(path: string, intent: string): boolean {
