...
vi.mock('../../scripts/core/state.js', () => ({
    readState: vi.fn((path) => {
        if (stateStore[path])
            return { ok: true, data: stateStore[path] };
...
vi.mock('../../scripts/guardrail/denial-tracker.js', () => {
    const denials = [];
    return {
        recordDenial: vi.fn((entry) => { denials.push(entry); }),
...
import { writeRealityCheckFlag, readRealityCheckFlag, clearRealityCheckFlag, } from '../../scripts/hooks/reality-check.js';
...
const denialMock = await import('../../scripts/guardrail/denial-tracker.js');
// ---------------------------------------------------------------------------
// Audit trail counter
// ---------------------------------------------------------------------------
let passCount = 0;
...
describe('Scenario 1: Guardrail block -> denial record', () => {
    it('checkBashCommand detects rm -rf and calls recordDenial', () => {
        ...
        expect(denialMock.recordDenial).toHaveBeenCalledWith(expect.objectContaining({ ruleId: 'warn-rm-rf', toolName: 'Bash' }), undefined);
    ...
    it('checkFilePath blocks .env and records denial', () => {
        ...
        expect(denialMock.recordDenial).toHaveBeenCalledWith(expect.objectContaining({ ruleId: 'protect-env', action: 'block' }), undefined);
...
describe('Scenario 4: PDCA scaling profile -> correct reviewers', () => {
    ...
    it('startPdca with complexityScore attaches correct scaling profile', () => {
        ...
        const state = stateStore[statePath];
...
describe('Scenario 5: Compaction preserves high priority, drops low priority', () => {
    it('PDCA_STATE (priority=100) is preserved, low-priority items are dropped under budget', () => {
        ...
        vi.mocked(getConfig).mockImplementation((path, fallback) => {
            if (path === 'context.maxCompactionTokens')
                return 80;
            return fallback;
        });
...
//# sourceMappingURL=harness-integration.test.js.map