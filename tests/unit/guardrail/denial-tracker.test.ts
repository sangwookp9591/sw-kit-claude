...
import { recordDenial, getDenialSummary, resetDenialState, DenialEntry } from '../../../scripts/guardrail/denial-tracker.js';
...
const sampleDenial: DenialEntry = {
...
describe('recordDenial', () => {
  ...
  it('increments sessionDenials in state', () => {
    ...
    const writtenState = mockWriteState.mock.calls[0][1] as { sessionDenials: number };
  ...
  it('adds new ruleId to topRules if not present', () => {
    mockReadState.mockReturnValue({ sessionDenials: 0, topRules: [], lastDenialAt: '' });
    recordDenial(sampleDenial, '/tmp/test-project');
    const writtenState = mockWriteState.mock.calls[0][1] as { topRules: Array<{ ruleId: string; count: number }> };
...
describe('resetDenialState', () => {
  it('writes default empty state', () => {
    ...
    const state = mockWriteState.mock.calls[0][1] as { sessionDenials: number; topRules: unknown[] };
