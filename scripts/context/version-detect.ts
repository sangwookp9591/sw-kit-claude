...
function gitFileDate(projectDir: string, filePath: string): string | null {
  try {
    if (!existsSync(join(projectDir, filePath))) return null;
    const out = execFileSync('git', ['log', '-1', '--format=%aI', '--', filePath], { cwd: projectDir, encoding: 'utf-8', timeout: 5000 }).trim();
    return out || null;
  } catch { return null; }
}

function getDepFileDates(projectDir: string): Record<string, string> {
  const DEP_FILES = [
...
function cachePath(dir: string): string { return join(dir, '.aing', 'state', 'tech-stack.json'); }

function isCacheValid(dir: string): boolean {
  const cache = readStateOrDefault(cachePath(dir), null) as TechStackCache | null;
  if (!cache?.detectedAt) return false;
  const current = getDepFileDates(dir);
  for (const [file, commitDate] of Object.entries(current)) {
    if (!cache.depFileCommits[file] || commitDate > cache.depFileCommits[file]) return false;
  }
  for (const file of Object.keys(current)) {
    if (!cache.depFileCommits[file]) return false;
  }
  return true;
}
...
function detectNode(dir: string, out: TechStackVersion[]): void {
  ...
  try {
    ...
    for (const name of NODE_PKGS) {
      ...
      if (!isNaN(major)) out.push({ name, version: clean, major, ecosystem: 'node' });
    }
  } catch { /* skip */ }
}
...
function detectPython(dir: string, out: TechStackVersion[]): void {
  ...
  if (existsSync(pyproj)) {
    try {
      ...
      for (const pkg of PY_PKGS) {
        ...
        if (m) out.push({ name: pkg, version: m[1], major: parseInt(m[1],10), ecosystem: 'python' });
      }
    } catch { /* skip */ }
  }
}
...
function detectJava(dir: string, out: TechStackVersion[]): void {
  // pom.xml
  ...
  if (existsSync(pom)) {
    try {
      ...
      for (const [re, name] of patterns) {
        const m = c.match(re);
        if (m) out.push({ name, version: m[1], major: parseInt(m[1],10), ecosystem: 'java' });
      }
    } catch { /* skip */ }
  }
  // build.gradle(.kts)
  for (const gf of ['build.gradle','build.gradle.kts']) {
    ...
    try {
      ...
      while ((m = pluginRe.exec(c)) !== null) {
        if (m[1].includes('spring')) out.push({ name: 'Spring Boot', version: m[2], major: parseInt(m[2],10), ecosystem: 'java' });
        if (m[1].includes('kotlin')) out.push({ name: 'Kotlin', version: m[2], major: parseInt(m[2],10), ecosystem: 'java' });
      }
      // sourceCompatibility / jvmTarget
      const javaM = c.match(/(?:sourceCompatibility|jvmTarget)\s*=?\s*['"]?(\d+)/);
      if (javaM) out.push({ name: 'Java', version: javaM[1], major: parseInt(javaM[1],10), ecosystem: 'java' });
    } catch { /* skip */ }
  }
}
...
function detectGo(dir: string, out: TechStackVersion[]): void {
  const p = join(dir, 'go.mod');
  if (!existsSync(p)) return;
  try {
    const c = readFileSync(p,'utf-8');
    const goV = c.match(/^go\s+([\d.]+)/m);
    if (goV) out.push({ name: 'Go', version: goV[1], major: parseInt(goV[1],10), ecosystem: 'go' });
    const goPkgs = ['gin','echo','fiber','chi','gorm','ent','cobra','viper','zap','zerolog'];
    for (const line of c.split('\n')) {
      for (const pkg of goPkgs) {
        if (line.includes(pkg)) {
          const m = line.match(/v([\d.]+)/);
          if (m) out.push({ name: pkg, version: m[1], major: parseInt(m[1],10), ecosystem: 'go' });
        }
      }
    }
  } catch { /* skip */ }
...
function detectRust(dir: string, out: TechStackVersion[]): void {
  ...
  try {
    ...
    for (const pkg of pkgs) {
      ...
      const m = c.match(re);
      if (m) out.push({ name: pkg, version: m[1], major: parseInt(m[1],10), ecosystem: 'rust' });
    }
    const ed = c.match(/edition\s*=\s*["'](\d+)["']/);
    if (ed) out.push({ name: 'Rust edition', version: ed[1], major: parseInt(ed[1],10), ecosystem: 'rust' });
  } catch { /* skip */ }
}
...
function detectRuby(dir: string, out: TechStackVersion[]): void {
  ...
  try {
    ...
    for (const pkg of pkgs) {
      ...
      if (m) out.push({ name: pkg, version: m[1], major: parseInt(m[1],10), ecosystem: 'ruby' });
    }
  } catch { /* skip */ }
}
...
function detectFlutter(dir: string, out: TechStackVersion[]): void {
  const p = join(dir, 'pubspec.yaml');
  if (!existsSync(p)) return;
  try {
    const c = readFileSync(p,'utf-8');
    const sdk = c.match(/sdk:\s*["']>=?([\d.]+)/);
    if (sdk) out.push({ name: 'Dart SDK', version: sdk[1], major: parseInt(sdk[1],10), ecosystem: 'dart' });
    const pkgs = ['riverpod','bloc','provider','dio','go_router','freezed','hive','drift','firebase_core'];
    for (const pkg of pkgs) {
      const m = c.match(new RegExp(`${pkg}:\\s*[\\^~]?([\\d.]+)`));
      if (m) out.push({ name: pkg, version: m[1], major: parseInt(m[1],10), ecosystem: 'dart' });
    }
  } catch { /* skip */ }
}
...
function detectPHP(dir: string, out: TechStackVersion[]): void {
  const p = join(dir, 'composer.json');
  if (!existsSync(p)) return;
  try {
    const pkg = JSON.parse(readFileSync(p,'utf-8'));
    const deps = { ...pkg.require, ...pkg['require-dev'] };
    const map: Record<string,string> = { 'laravel/framework':'Laravel', 'symfony/symfony':'Symfony', 'php':'PHP', 'phpunit/phpunit':'PHPUnit', 'doctrine/orm':'Doctrine' };
    for (const [n, label] of Object.entries(map)) {
      const v = deps[n];
      if (v) { const c = v.replace(/^[\^~>=<|]+/,''); const maj = parseInt(c,10); if (!isNaN(maj)) out.push({ name: label, version: c, major: maj, ecosystem: 'php' }); }
    }
  } catch { /* skip */ }
}
...
function detectDotnet(dir: string, out: TechStackVersion[]): void {
  // Find .csproj
  try {
    ...
    if (tfm) { const v = tfm[1].replace('net',''); out.push({ name: '.NET', version: v, major: parseInt(v,10), ecosystem: 'dotnet' }); }
  } catch { /* skip */ }
}
...
export function detectVersions(projectDir: string): DetectedStack {
  ...
  const deduped = out.filter(v => { if (seen.has(v.name)) return false; seen.add(v.name); return true; });
...
export function detectVersionsCached(projectDir: string): DetectedStack {
  if (isCacheValid(projectDir)) {
    const cache = readStateOrDefault(cachePath(projectDir), null) as TechStackCache | null;
    if (cache) return { versions: cache.versions, summary: cache.summary, fromCache: true };
  }
  const result = detectVersions(projectDir);
  // Save cache
  mkdirSync(join(projectDir, '.aing', 'state'), { recursive: true });
  const cache: TechStackCache = { detectedAt: new Date().toISOString(), versions: result.versions, summary: result.summary, depFileCommits: getDepFileDates(projectDir) };
...
type WarnRule = { match: (v: TechStackVersion) => boolean; warn: (v: TechStackVersion) => string };

const WARNINGS: WarnRule[] = [
  // Node/JS
...
export function generateVersionContext(projectDir: string): string {
  ...
  for (const v of versions) {
    for (const r of WARNINGS) { if (r.match(v)) { lines.push(`⚠ ${r.warn(v)}`); break; } }
  }
  lines.push('');
