...
export interface DenialEntry {
  ...
  severity: string;
  message: string;
  input?: string;   // truncated tool input for context
}

interface DenialAuditState {
 ...
 */
export function recordDenial(entry: DenialEntry, projectDir?: string): void {
  ...
  try {
    ...
    const state = readStateOrDefault(statePath, DEFAULT_STATE) as DenialAuditState;
 ...
 */
export function getDenialSummary(projectDir?: string): string | null {
  const dir = projectDir || process.cwd();
  const statePath = join(dir, '.aing', 'state', 'denial-audit.json');
  const state = readStateOrDefault(statePath, DEFAULT_STATE) as DenialAuditState;

  if (state.sessionDenials === 0) return null;
 ...
 */
export function resetDenialState(projectDir?: string): void {
  const dir = projectDir || process.cwd();
  const statePath = join(dir, '.aing', 'state', 'denial-audit.json');
