 ...
 */
function countCompletedTasks(projectDir: string): number {
  try {
    const indexPath = join(projectDir, '.aing', 'tasks', '_index.json');
    if (!existsSync(indexPath)) return 0;
    const raw = readFileSync(indexPath, 'utf-8');
    const entries = JSON.parse(raw) as Array<{ status?: string }>;
 ...
 */
function hasAllSections(filePath: string): boolean {
  try {
 ...
 */
function findLatestReport(reportsDir: string): string | null {
  try {
 ...
 */
export function checkCompletionReport(projectDir?: string): ReportCheckResult {
