...
const log = createLogger('review-checklist');
...
export const CATEGORIES = {
  ...
  'sql-safety': {
    ...
    patterns: [
      ...
      { regex: /(?:query|exec|raw)\s*\(\s*['"`].*\+/gi, desc: 'String concatenation in query' },
  ...
  'llm-trust-boundary': {
    ...
    patterns: [
      { regex: /dangerouslySetInnerHTML|v-html|innerHTML\s*=/gi, desc: 'Unsanitized HTML rendering' },
      // Detects eval() called on LLM/AI response variables
      { regex: /eval\s*\([^)]*(?:response|output|result|completion)/gi, desc: 'Code evaluation on LLM output' },
      { regex: /system.*prompt.*\$\{.*user/gi, desc: 'User input in system prompt' },
    ],
  },
  'enum-completeness': {
    pass: 1,
  ...
  'async-sync-mixing': {
    ...
    patterns: [
      { regex: /(?:readFileSync|writeFileSync|execSync)\s*\(/g, desc: 'Sync I/O in potentially async context' },
    ],
  },
  'crypto-entropy': {
    pass: 2,
 ...
 * @returns {Array<{ category: string, severity: string, pass: number, findings: Array<{ desc: string, matches: number, lines: string[] }> }>}
 */
 ...
 * @param {Array} results - Output of runChecklist()
 * @returns {{ autoFix: Array, needsAsk: Array, summary: object }}
 */
