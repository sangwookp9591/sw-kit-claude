...
function errorPath(projectDir: string): string {
 ...
 */
function errorSignature(errorMsg: string): string {
  return errorMsg
...
export function recordToolError(
  ...
  toolName: string,
  errorMsg: string
): { guidance: string | null; forceAlternative: boolean } {
  const sig = errorSignature(errorMsg);
  const now = new Date();

  const stateResult = readState(errorPath(projectDir));
  const state: ErrorRecoveryState = stateResult.ok
    ? stateResult.data as ErrorRecoveryState
 ...
 */
export function clearToolErrors(projectDir: string, toolName: string): void {
  const stateResult = readState(errorPath(projectDir));
  if (!stateResult.ok) return;

  const state = stateResult.data as ErrorRecoveryState;
