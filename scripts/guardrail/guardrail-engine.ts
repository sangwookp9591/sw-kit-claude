 ...
 */
export function loadRules(projectDir?: string): GuardrailRule[] {
  ...
  let escalatedOverrides = new Map<string, 'block'>();
  try {
  ...
  return allRules.map(r => {
    ...
    if (escalated && r.action === 'warn') return { ...r, action: escalated };
