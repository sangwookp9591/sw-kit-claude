 ...
 */
function normalizeDirPath(directory: string): string {
...
export function setFreeze(directory: string, projectDir?: string): FreezeResult {
  ...
  if (existsSync(allowlistPath)) unlinkSync(allowlistPath);
 ...
 */
export function setFreezeAllowList(directories: string[], projectDir?: string): FreezeResult {
  const dir = projectDir || process.cwd();
  const stateDir = join(dir, '.aing', 'state');
  mkdirSync(stateDir, { recursive: true });
 ...
 */
export function getFreezeMode(projectDir?: string): { mode: FreezeMode; paths: string[] } {
  const dir = projectDir || process.cwd();
  const stateDir = join(dir, '.aing', 'state');
  ...
  if (existsSync(allowlistPath)) {
    try {
      const paths = JSON.parse(readFileSync(allowlistPath, 'utf-8')) as string[];
  ...
  if (existsSync(singlePath)) {
    try {
      ...
      if (freezeDir) return { mode: 'single', paths: [freezeDir] };
    } catch { /* fall through */ }
  }
