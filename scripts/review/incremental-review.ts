 ...
 */
export function getChangedFiles(projectDir: string, baseBranch: string = 'HEAD~1'): string[] {
  try {
 ...
 */
export function filterReviewScope(
  allFiles: string[],
  changedFiles: string[],
): IncrementalResult {
  ...
  for (const file of allFiles) {
    ...
    const isChanged =
      changedSet.has(file) ||
      [...changedSet].some(
        changed => file.endsWith(changed) || changed.endsWith(file),
      );
