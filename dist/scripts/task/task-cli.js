 ...
 */
import { createTask, checkSubtask, listTasks, addSubtask, formatTaskChecklist, formatAllTasks, } from './task-manager.js';
...
switch (command) {
    case 'create': {
        ...
        const subtaskTitles = (args.subtasks || '')
            ...
            .map(s => s.trim())
            .filter(Boolean);
        const result = createTask({ title, subtasks: subtaskTitles.map(t => ({ title: t })) }, dir);
        if (result.ok) {
