...
vi.mock('../../../scripts/routing/complexity-scorer.js', () => ({
    scoreComplexity: vi.fn((signals = {}) => {
        const fileCount = signals.fileCount || 0;
        const domainCount = signals.domainCount || 0;
        const lineCount = signals.lineCount || 0;
        let files = 0;
        if (fileCount > 15)
            files = 3;
        else if (fileCount > 5)
...
import { selectTeamWithProfile, selectTeam, } from '../../../scripts/pipeline/team-orchestrator.js';
...
describe('selectTeamWithProfile — additional edge cases', () => {
    ...
    it('all categories disabled → returns solo fallback worker', () => {
        ...
        const profile = makeProfile({
            allowedAgents: new Set(['sam']), // only sam, no executor allowed
            maxTeamSize: 10,
        });
...
//# sourceMappingURL=team-orchestrator-profile.test.js.map