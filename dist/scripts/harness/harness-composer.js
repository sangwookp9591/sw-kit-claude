 ...
 */
export function autoCompose(task, _projectDir, patterns) {
    ...
    const stages = [];
    if (/research|조사|분석|survey|report/.test(lower))
        stages.push('research');
    if (/design|설계|architect|blueprint/.test(lower))
        stages.push('design');
    stages.push('build');
    if (/test|테스트|qa|quality/.test(lower))
        stages.push('test');
    if (/review|리뷰|audit|check/.test(lower))
        stages.push('review');
    if (/deploy|배포|ship|release/.test(lower))
        stages.push('deploy');
