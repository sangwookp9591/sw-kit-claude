 ...
 */
function detectAnchors(input) {
    const anchorTypes = [];
    if (FILE_PATH_PATTERNS.some(p => p.test(input)))
        anchorTypes.push('file');
    if (SYMBOL_PATTERNS.some(p => p.test(input)))
        anchorTypes.push('symbol');
    if (ISSUE_REF_PATTERN.test(input))
        anchorTypes.push('issue');
    if (ERROR_PATTERNS.some(p => p.test(input)))
        anchorTypes.push('error');
    if (CODE_BLOCK_PATTERN.test(input))
        anchorTypes.push('codeblock');
    if (NUMBERED_LIST_PATTERN.test(input))
        anchorTypes.push('list');
 ...
 */
function estimateComplexity(input, _anchorInfo) {
    ...
    const domainCount = domainKeywords.filter(domains => domains.some(kw => inputLower.includes(kw))).length || 1;
...
function resolvePreset(route, complexityScore, isDesign) {
    if (isDesign)
        return 'design';
    if (route === 'team') {
