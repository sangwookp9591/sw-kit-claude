 ...
 */
export function analyzeDenials(projectDir) {
    ...
    if (output.escalations.length > 0) {
        output.contextInjection.push(`[Denial Learning] ${output.escalations.length} rule(s) auto-escalated (warn → block):`);
        for (const esc of output.escalations) {
            output.contextInjection.push(`  - ${esc.ruleId}: ${esc.count}x → escalated to block. ${esc.reason}`);
        }
    }
 ...
 */
export function getEscalatedRules(projectDir) {
    const dir = projectDir || process.cwd();
    const outputPath = join(dir, '.aing', 'state', 'denial-learner-output.json');
    const overrides = new Map();
    const output = readStateOrDefault(outputPath, { escalations: [] });
    for (const esc of output.escalations || []) {
        overrides.set(esc.ruleId, 'block');
    }
    return overrides;
}
//# sourceMappingURL=denial-learner.js.map