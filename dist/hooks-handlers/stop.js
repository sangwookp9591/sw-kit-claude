...
try {
    ...
    try {
        ...
        if (!isPdcaActive) {
            ...
            if (denials) {
                capturePassive({ trigger: 'session-end', content: `세션 종료 시 가드레일 기록: ${denials.slice(0, 120)}`, context: 'session' }, projectDir);
            }
            capturePassive({ trigger: 'session-end', content: `비 PDCA 세션 종료 (${new Date().toISOString().slice(0, 10)})`, context: 'session' }, projectDir);
        }
    }
    ...
    try {
        ...
        if (existsSync(claudeMdPath)) {
            ...
            if (content.includes(marker)) {
                const newContent = content.replace(/> 마지막 갱신: .+/, `> 마지막 갱신: ${updatedAt}`);
                // Only add update line if not already present
                if (newContent === content && !content.includes('> 마지막 갱신:')) {
                    const withUpdate = content.replace(marker, `${marker}\n> 마지막 갱신: ${updatedAt}`);
