...
try {
  ...
  const isPlanOrReview: boolean = ['plan', '계획', '기획', '설계', 'review', '리뷰', '코드리뷰'].some(kw => lower.includes(kw));
  if (isPlanOrReview) {
    try {
  ...
  const isExecutionIntent: boolean = /implement|build|execute|실행|구현|만들|개발|추가|수정|fix|refactor/i.test(prompt);
  const hasPlanSession: boolean = (() => {
    try {
